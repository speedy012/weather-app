...
ChartJS.register({ id: 'line', component: Line })

export const ForecastCard = ({ currDate }: DefaultLocationCardProps) => {
  ...
  const utcTime = format(new Date(currentConditions.datetimeEpoch * 1000), 'E, MMMM do, yyyy')
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-lg border shadow-sm bg-white p-6">
      <p className="text-sm font-medium text-gray-60 text-wrap text-center w-56">{currDate.datetime ?? utcTime}</p>
      ...
      <div className="flex gap-2">
        ...
        <div className="mt-4">
          ...
          <div className="flex  items-center">
            ...
            <p className="text-xl font-medium text-gray-600">
              {currDate.precip === 0 ? 'No rain' : `${currDate.precip ?? currentConditions.precip} %`}
            </p>
          </div>
        </div>
      </div>
